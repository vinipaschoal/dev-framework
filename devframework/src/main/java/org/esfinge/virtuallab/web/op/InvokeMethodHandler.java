...
import org.esfinge.virtuallab.descriptors.FormDescriptor;
...
public class InvokeMethodHandler implements IJsonRequestHandler
{
	public JsonObject handleAsync(HttpServletRequest request) throws FileNotFoundException
	{
		...
		try
		{
			...
			Method method = PersistenceService.getInstance().getMethod(clazzRequest.getQualifiedName(),
					clazzRequest.getMethods().get(0));
